...
import {
  ...
  PLAYER_SLUGS, ALL_PLAYERS, H2H_FIXTURES, FINAL_ROUND, picksRequired,
...
export default function App() {
  ...
  const handleSelectNation = (nation) => {
    ...
    if (selectedSlot < selections.length-1 && !updated[selectedSlot+1]) setSelectedSlot(selectedSlot+1);
  };
              ...
              {!roundOpen ? (
                ...
                <>
                  {activeRound === FINAL_ROUND && (
                    <div className="mb-4 px-3 py-2 bg-[#007AFF]/10 border border-[#007AFF]/30 rounded-xl flex items-center gap-2">
                      <span className="text-sm">🏆</span>
                      <p className="text-xs font-semibold text-[#007AFF]">Final week — the Final and the 3rd-place playoff. Just 2 picks this time, and the armband still counts.</p>
                    </div>
                  )}
                  {/* Slots */}
                  <div className={`grid grid-cols-1 gap-3 mb-4 ${selections.length === 2 ? 'md:grid-cols-2' : 'md:grid-cols-3'}`}>
                ...
                <div className="p-5 border-t border-[#E5E5EA] bg-white space-y-4 text-xs text-[#636366] leading-relaxed">
                  ...
                  <div className="bg-gradient-to-br from-[#007AFF]/5 to-[#5AC8FA]/5 border border-[#007AFF]/15 rounded-xl p-4">
                    ...
                    <ul className="space-y-1.5 ml-1">
                      <li className="flex gap-2"><span>🎯</span><span>Each gameweek you pick <strong>3 nations</strong> from that round's matches (<strong>2 nations</strong> in the final week).</span></li>
                  ...
                  <div><h4 className="font-bold text-[#1C1C1E] mb-1">1. Triple Pick</h4><p>Select exactly 3 nations from the active match pool each gameweek. The final week (GW8) covers just the Final and the 3rd-place playoff — 4 nations — so you pick <span className="font-bold text-black">2 nations</span> there instead, still with an armband.</p></div>
